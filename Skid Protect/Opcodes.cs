...
namespace Skid_Protect
{
    static class Opcodes
    {
        static public Dictionary<int, string> ops = new Dictionary<int, string>()
        {
            ...
            { 1 , "stack[instruction.A] = constants[instruction.Bx].constants_data;" },
        ...
        for i = instruction.A, instruction.B do
            ...
            {5, @"local key = constants[instruction.Bx].constants_data;
        stack[instruction.A] = environment[key];" },
            {6, @"local C = instruction.C
        local stack = stack
        C = C > 255 and constants[C-256].constants_data or stack[C]
        stack[instruction.A] = stack[instruction.B][C];" },
            {7,@"local key = constants[instruction.Bx].constants_data;
        ...
        B = B > 255 and constants[B-256].constants_data or stack[B];
        C = C > 255 and constants[C-256].constants_data or stack[C];
        ...
        A = A ~= 0
        if (B > 255) then B = constants[B-256].constants_data else B = stack[B] end
        if (C > 255) then C = constants[C-256].constants_data else C = stack[C] end
        if (B == C) ~= A then
            IP = IP + 1
        end" },
